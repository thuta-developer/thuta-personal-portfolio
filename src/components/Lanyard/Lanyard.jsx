...
function Band({ maxSpeed = 50, minSpeed = 0 }) {
  ...
  useSphericalJoint(j3, card, [[0, 0, 0], [0, 3.50, 0]]);
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current) {
      ...
      // ကြိုးအစီအစဉ်ကို Fixed point မှ Card ဆီသို့ အစဉ်လိုက်ဖြစ်အောင် ပြင်ဆင်ထားသည်
  ...
  return (
    <>
      <group position={[4, 7.50, 0]}> 
        ...
        <RigidBody position={[0, -6, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
          <CuboidCollider args={[2, 2.8, 0.05]} /> 
          <group
            scale={3.15}
            position={[0, -2.8, -0.05]}
      ...
      <mesh ref={band}>
        ...
        <meshLineMaterial
          ...
          map={texture}
          repeat={[-4, 1]}
          lineWidth={0.8} // Scale 5 နဲ့ လိုက်အောင် ကြိုးကို အနည်းငယ် ပိုထူပေးထားသည်
        />
      </mesh>
    </>
  );
